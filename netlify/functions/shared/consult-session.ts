...
export async function endConsultSession(
  ...
  const userRef = deps.db.collection("users").doc(uid);
  const consultationRef = userRef.collection("consultations").doc(payload.sessionId);

  const result: ConsultEndTransactionResult = await deps.db.runTransaction(async (tx) => {
    const userSnap = await tx.get(userRef);
    ...
    const startedAtMs =
      typeof consultation.startedAtMs === "number" ? consultation.startedAtMs : 0;
    if (!startedAtMs) {
      throw new ConsultSessionError("Consultation session is missing start time", 400);
    }
    ...
    await applyCreditChangeInTransaction(
      ...
      userRef,
      {
        ...
        ledgerId: `consultation_${payload.sessionId}`,
        metadata: {
          personaId: persona.id,
          minutes,
          durationSeconds,
    ...
    tx.update(consultationRef, {
      ...
      pricePerMin,
      maxBillableMinutes,
      messageCount: consultation.messageCount ?? payload.messageCount,
      updatedAt: deps.FieldValue.serverTimestamp(),
    });
