...
import { Heart, Loader2, Activity, Zap, Brain } from "lucide-react";
...
export default function Wellness() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      postJson(
        "/api/kundali",
        { birthData: requestBirthData, chartType: "BIORHYTHMS" },
      ...
      postJson(
        "/api/kundali",
        { birthData: requestBirthData, chartType: "WELLNESS_SCORE" },
        { signal: controller.signal },
      )
        .then((r) => r.json())
        .catch(() => null),
      postJson(
        "/api/kundali",
        { birthData: requestBirthData, chartType: "ENERGY_PATTERNS" },
        { signal: controller.signal },
      )
        .then((r) => r.json())
        .catch(() => null),
    ...
    <PageShell>
      <div>
        ...
        <p className="text-white/50 mb-8">
          Your cosmic energy cycles and health insights
        </p>
        ...
        {!requestBirthData?.dob ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {biorhythms ? (
                <div className="space-y-4">
                  {[
                    ...
                    const val =
                      biorhythms[cycle.key] ??
                      biorhythms[`${cycle.key}_cycle`] ??
                      0;
                    const pct =
                      typeof val === "number"
                    ...
                    return (
                      <div key={cycle.key}>
                        ...
                        <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${cycle.color}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                      </div>
                ...
                <div className="space-y-3">
                  {["Physical rhythm", "Emotional rhythm", "Mental rhythm"].map(
                    (label) => (
                      <div key={label}>
              ...
              {wellnessScore ? (
                <>
                  ...
                  <p className="text-white/40 text-sm">
                    {wellnessScore.interpretation ||
                      wellnessScore.description ||
                      "Overall cosmic wellness"}
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-white/40 text-sm">
                    Baseline wellness view while live chart scoring refreshes.
                  </p>
                </>
              )}
            </div>
            ...
            <div className="glass rounded-[2rem] p-6">
              ...
              {energy ? (
                <div className="space-y-3">
                  {(energy.patterns || energy.periods || [])
                    .slice(0, 6)
                    .map((p: any, i: number) => (
                      <div
                        key={i}
                      ...
                      >
                        <span className="text-amber-400 text-xs min-w-[60px]">
                          {p.time || p.period}
                        </span>
                        <p className="text-white/60 text-xs">
                          {p.description || p.energy_level || p.advice}
                        </p>
                      </div>
                    ))}
                </div>
                ...
                <div className="space-y-3">
                  {[
                    [
                      "Morning",
                      "Use the clearest energy for one important task.",
                    ],
                    [
                      "Afternoon",
                      "Keep decisions practical and avoid overloading the calendar.",
                    ],
                    ...
                    <div
                      ...
                      className="flex items-start gap-2 py-1.5 border-b border-white/5 last:border-0"
                    >
                      <span className="text-amber-400 text-xs min-w-[70px]">
                        {time}
                      </span>
