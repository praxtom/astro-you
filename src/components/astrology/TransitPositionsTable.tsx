...
const TransitPositionsTable: React.FC<TransitPositionsTableProps> = ({
  data,
}) => {
  ...
  const filtered = positions.filter((p: any) =>
    mainPlanets.some(
      (m) => p.name === m || p.name?.replace("_transit", "") === m,
    ),
  );
        ...
        {display.map((p: any) => {
          ...
          return (
            ...
            >
              <span
                className="text-lg"
                style={{ color: planetInfo?.color || "#FFD700" }}
              >
              ...
              <span className="text-[11px] text-white/50">{signFull}</span>
              {degree && (
